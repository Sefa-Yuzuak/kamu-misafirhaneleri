...
{komsu_html}{gezi_html}"""
    ...
    return kabuk(
        ...
        anahtarlar=[
            ...
            f"{t['il']} kamu misafirhanesi", t["tur"].lower(),
            *( [f"{kisa_ad(t['ad'])} fiyat", f"{t['il']} öğretmenevi fiyatları"]
               if t.get("fiyat_2026") else [] ),
            *( [f"{t['il']} denize yakın öğretmenevi"] if t.get("deniz") else [] ),
        ],
...
{sss_html(sss)}
    ...
    return kabuk(
        ...
        anahtarlar=[
            ...
            f"{il} konaklama", f"{il} memur tatili",
            *( [f"{il} denize yakın kamu tesisi"] if deniz else [] ),
        ],
        kirintilar=kirintilar,
        aktif="/il/",
        harita=haritali,
